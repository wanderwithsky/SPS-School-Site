import { Mail, Phone } from "lucide-react";
import { Link } from "@tanstack/react-router";

/* Dummy contact data — replace later without changing structure */
export const RIBBON = {
  email: "71349@cbseshiksha.in",
  phone: "+91-8765261262 ",
  whatsapp: "+91-9305999803",
  whatsappHref: "https://wa.me/9305999803",
  phoneHref: "tel:+919305999803",
  instagram: "https://instagram.com/yourschool",
  admissionLabel: "Admission Open 2026-27 🎉",
} as const;

function WhatsAppGlyph({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884a9.82 9.82 0 0 1 6.988 2.896 9.83 9.83 0 0 1 2.892 6.994c-.003 5.45-4.437 9.886-9.884 9.886m8.413-18.297A11.82 11.82 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.88 11.88 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.82 11.82 0 0 0 20.464 3.488" />
    </svg>
  );
}

const linkCls =
  "flex shrink-0 items-center gap-1.5 whitespace-nowrap text-white/90 transition duration-200 hover:text-white";

export function TopRibbon() {
  return (
    <div className="w-full bg-[#A6192E] text-white">
      <div className="mx-auto flex max-w-7xl items-center gap-x-6 overflow-x-auto px-4 py-1.5 text-[11.5px] font-medium tracking-[0.04em] [scrollbar-width:none] sm:px-6 [&::-webkit-scrollbar]:hidden">
        <a href={`mailto:${RIBBON.email}`} className={linkCls}>
          <Mail className="size-3.5" aria-hidden="true" />
          {RIBBON.email}
        </a>
        <a href={RIBBON.phoneHref} className={linkCls}>
          <Phone className="size-3.5" aria-hidden="true" />
          {RIBBON.phone}
        </a>
        <a href={RIBBON.whatsappHref} target="_blank" rel="noreferrer noopener" className={linkCls}>
          <WhatsAppGlyph className="size-3.5" />
          {RIBBON.whatsapp}
        </a>
        <Link to="/contact" className={`${linkCls} hidden md:flex`}>
          Contact
        </Link>

        <button
          type="button"
          aria-label="Open Admissions 2026-27"
          onClick={() => window.dispatchEvent(new Event("open-admission-popup"))}
          className="admission-pulse ml-auto shrink-0 whitespace-nowrap pl-4 text-[12.5px] font-semibold tracking-[0.1em] uppercase transition-transform duration-300 ease-out hover:scale-[1.03] hover:brightness-110 focus:outline-none"
        >
          {RIBBON.admissionLabel}
        </button>
      </div>
    </div>
  );
}
