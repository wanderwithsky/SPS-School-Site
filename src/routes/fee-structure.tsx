          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center pt-8 pb-16 sm:pt-12 sm:pb-24">
            ...
            <div className="max-w-2xl text-center lg:text-left mx-auto lg:mx-0">
              ...
              <motion.h1
                ...
                className="mt-4 font-serif text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground leading-tight"
              >
                Fee Structure
              </motion.h1>
              <motion.p
        ...
        <section className="mx-auto max-w-4xl px-5 sm:px-6 mt-20 text-center">
          ...
          >
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
              We believe parents should have clear and accessible information about the cost of
              education. View the applicable fee structure for the current academic session below.
            </p>
            ...
            <div className="mt-12 inline-flex flex-col items-center justify-center rounded-2xl bg-muted/30 px-10 py-6 border border-border">
              ...
              <span className="mt-2 font-serif text-4xl font-bold text-primary">
                {FEE_DOCUMENT.session}
              </span>
            </div>
          </motion.div>
        </section>

        {/* 3. Interactive Class-Wise Fee Schedule Table */}
        <section className="mx-auto max-w-5xl px-5 sm:px-6 mt-16">
          <motion.div
            initial={{ opacity: 0, y: 30 }}
          ...
          >
            <div className="p-6 sm:p-8 border-b border-border bg-muted/20">
              ...
              <h3 className="mt-2 font-serif text-2xl font-bold text-foreground">
                Class-Wise Tuition &amp; Admission Schedule
              </h3>
              <p className="mt-1 text-sm text-muted-foreground">
                Official rates approved by school management for session 2026–27
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-foreground">
                <thead className="bg-muted/40 text-xs font-semibold uppercase tracking-wider text-muted-foreground border-b border-border">
                  <tr>
                    <th className="px-6 py-4">Wing / Category</th>
                ...
                <tbody className="divide-y divide-border">
                  {feeTiers.map((tier) => (
                    <tr key={tier.id} className="hover:bg-muted/20 transition-colors">
                      <td className="px-6 py-4 font-semibold text-primary">{tier.category}</td>
                      <td className="px-6 py-4 font-medium">
                        {tier.class_name}
                        {tier.notes && (
                          <p className="text-xs text-muted-foreground mt-0.5">{tier.notes}</p>
                        )}
                      </td>
                      <td className="px-6 py-4">{tier.admission_fee}</td>
                      <td className="px-6 py-4 font-bold text-foreground">{tier.tuition_fee}</td>
                      <td className="px-6 py-4 text-muted-foreground">{tier.annual_charges}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-8 border-b border-border">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Official Document • Last Updated: {FEE_DOCUMENT.lastUpdated}
                </p>
              </div>
